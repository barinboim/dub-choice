...
export interface DubPack {
  ...
  warnings: string[];
  /**
   * `source=` в _pack_info.ini (наше расширение): адрес, с которого игрок
   * взял видео в веб-студии. Нужен модерации галереи — по паку иначе не
   * понять, откуда сцена, а спрашивать это формой бессмысленно (см.
   * CLAUDE.md, «Аналитика»: анкете человек либо соврёт, либо бросит её).
   * Пустая строка — видео принесли файлом, источника мы не знаем.
   */
  sourceUrl: string;
  /**
