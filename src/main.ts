...
/**
 * «Скачать»/«Редактировать» на карточке пака доступны для любого пака —
 * встроенного, чужого ZIP или своего: скачать в формате TCV или открыть на
 * правку хочет не только автор (решение владельца 2026-09-05). На премьере
 * тот же блок оставлен только для пака, собранного в редакторе, — там это
 * действия с паком, а не с роликом, и встроенным/чужим ролик уже показан
 * ровно таким, какой он есть.
 */
function syncStudioPackButtons(): void {
  const own = currentPackSlug === "studio";
  $("final-own-pack").hidden = !own;
}
